...
struct Berth
{
    ...
    int berthId;
    queue<int> goodsList;
    bool operator<(const Berth& other) const { // 比较 value 值, value 越大优先级越高
...
struct Boat
{
    ...
    num 表示 船当前装了多少货物
    */
    int shipedFrame, id, status, num, myLastBerth, shipId;
    ...
    bool operator<(const Boat& other) const {   //船的优先级是 已经装的越多优先级越高
...
DequePriorityQueue<Goods> goods_queue;
DequePriorityQueue<Goods> temp_this_frame_goods;
...
void chooseRobot(Goods goods){
    ...
    if (robot[maxRoboId].myGoods.size() >= 10) {
        if (maxR > robot[maxRoboId].myGoods.getMin().ratio) {
            ...
            robot[maxRoboId].myGoods.removeMin();
        } 
    } else {
        if(maxR < threshold){
            goods_ignore ++;
            ...
            goods.ratio = maxR;
            robot[maxRoboId].myGoods.insert(goods);
        }
    }
}
...
int main() {
    ...
    for(int frame = 1; frame <=15000; frame ++){
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            int temp_num = temp_this_frame_goods.size();
            fout << "%%%%%%%%%%%%%% has " << temp_num << " dibao.\n";
            for (int k = 0; k < temp_num; k++) {
                ...
                if (maxId != -1) {
                    robot[maxId].myGoods.insert(t);
                    fout << "***********Robot: " << maxId << " 通过低保分配到了一个货物! waiting_frame is " << robot[maxId].waiting_frame<< endl;
                    temp_this_frame_goods.removeMax();
                }
            }
            while (!temp_this_frame_goods.isEmpty())
                temp_this_frame_goods.removeMax();
            ...
            if (robot[i].status == 0 && robot[i].st != 0 && !robot[i].has_goods && !robot[i].myGoods.isEmpty()){
                // 把前 1 个超时的货物 丢弃掉
                int flag = 1;
                while (flag > 0 && frame_id - robot[i].myGoods.getMax().frame >= 1000 && !robot[i].myGoods.isEmpty()) {
                    robot[i].myGoods.removeMax();
                    flag--;
                }
                if (!robot[i].myGoods.isEmpty()) { 
                    Goods tmp = robot[i].myGoods.getMax();
