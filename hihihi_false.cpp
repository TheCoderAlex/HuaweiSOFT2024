...
void chooseRobot(Goods goods){
    ...
    if(maxRoboId == -1) 
        ...
        goods.ratio = maxR;
        if(robot[maxRoboId].help_robot == maxRoboId){// 如果这个机器人没有在外帮忙，货物照常入队
            fout << "[@@@] robot " << maxRoboId << "'s myGoods +1" << endl;
            robot[maxRoboId].myGoods.push(goods);
        }else{// 如果机器人在外帮忙，要把货物放到他原本港口的临时队列中
            int maxBerthId = choice[maxRoboId];
            berth[maxBerthId].goodsAfterHelp.push(goods);
            fout << "[@@@] berth " << maxBerthId << "'s goodsAfterHelp +1" << endl;
            fout << "[@@@] berth " << maxBerthId << "'s goodsAfterHelp : " << berth[maxBerthId].goodsAfterHelp.size() << endl;
            
            //如果berth中的队列size达到20，则把这20个取出来放在mygoods中
            if (berth[maxBerthId].goodsAfterHelp.size() > 20){
                fout << "[@@@] robot " << maxRoboId << " is back to berth " << maxBerthId << endl;
                while (!berth[maxBerthId].goodsAfterHelp.empty()){
                    Goods g = berth[maxBerthId].goodsAfterHelp.front();
                    if (frame_id - g.frame <= 1000){
                        robot[maxRoboId].myGoods.push(g);
                        berth[maxBerthId].goodsAfterHelp.pop();
                    }else{
                        berth[maxBerthId].goodsAfterHelp.pop();
                    }
                }
                robot[maxRoboId].help_robot = maxRoboId;
            }
        }
    }
}
        ...
        for (int i = 0; i < 10; ++i) {    
            ...
            if (robot[i].status == 0 && robot[i].st != 0 && !robot[i].has_goods){
                ...
                while (!robot[i].myGoods.empty() && flag > 0 && frame_id - robot[i].myGoods.top().frame >= 1000) {
                    robot[i].myGoods.pop();
                    flag--;
                    fout << "[*]robot " << i  << " throwing a goods." << endl;
                }
                /* 几种状态梳理：
                  1. 自己没货，别人也不需要帮忙：robot[i].myGoods.empty() && robot[i].help_robot == i --》进入else代码块，什么也不处理
                  2. 自己没货，别人需要帮忙：robot[i].myGoods.empty() && robot[i].help_robot == i --》进入else代码块，获取help_robot
                  3. 正在帮忙，还可以继续帮忙：robot[i].myGoods.empty() --》 进入else代码块，继续获取help_robot
                  4. 正在帮忙，自己港口货物堆积了，需要返回：!robot[i].myGoods.empty() && robot[i].help_robot == i --》进入if代码块
                  5. 自己有货：!robot[i].myGoods.empty() && robot[i].help_robot == i --》进入if代码块
                */
                if (!robot[i].myGoods.empty() && robot[i].help_robot == i) { 
                    //如果队列不为空且他没有去帮别人干活。说明该robot对应的港口存在有效货物，那么还是交付到对应的港口
                    Goods tmp = robot[i].myGoods.top();
                    ...
                    // myGoods为空的情况：自己的港口的货物搬完了，小憩中
                    // help_robot != i的情况：在外面帮别人干活，自己港口还没有累积到一定程度的货物
                    // 这种情况下都应该让这个机器人去外面帮忙
                    int j = 1;
                    int n = 10;
                    ...
                    // 1. 找到符合条件的港口
                    while(j < n && robot[nearId].myGoods.size() <= 20){
